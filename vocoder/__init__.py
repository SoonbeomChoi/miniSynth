...
class HiFiGAN(torch.nn.Module):
    def __init__(self, mel_size=80):
        super(HiFiGAN, self).__init__()
        config = HiFiGANConfig()
        self.num_kernels = len(config.resblock_kernel_sizes)
        self.num_upsamples = len(config.upsample_rates)
        self.conv_pre = weight_norm(Conv1d(mel_size, config.upsample_initial_channel, 7, 1, padding=3))
...
def run(mel, vocoder_file='./vocoder/checkpoint.pt'):
    hifigan = HiFiGAN()
    hifigan.load_state_dict(torch.load(vocoder_file, map_location='cpu'))

    hifigan.eval()
    hifigan.remove_weight_norm()

    mel = -config.min_level_db*mel.clamp(0, 1) + config.min_level_db
    mel = math.log(10.0)*mel/20.0
    if mel.ndim < 3:
        mel = mel.unsqueeze(0)

    hifigan = hifigan.to(mel.device)
    with torch.no_grad():
        audio = hifigan(mel).squeeze(0)
